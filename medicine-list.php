        ...
        try {
          ...
          if (count($medicines) > 0) {
            foreach ($medicines as $medicine) {
              ...
              echo '</div>';
              echo '<button class="btn btn-primary add-to-cart-btn" data-id="' . $medicine['m_id'] . '">Add to Cart</button>';
              echo '</div></div></div>';
            }
          } else {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  addToCartButtons.forEach(button => {
    button.addEventListener('click', function() {
      ...
      const medicine = {
        id: medicineItem.dataset.id,
        name: medicineItem.dataset.name,
        price: parseFloat(medicineItem.dataset.price),
        quantity: 1
      };
